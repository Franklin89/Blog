...
Stripe offers a way where your system will not get in touch with any of that data and only work with tokens. So we do not have to worry about any of the PCI Compliances. Stripe offers a JavaScript library called `Stripe.js` that I will leverage so that a user can securely enter their payment details. If the user clicks on the link `Add your payment details now!` a modal dialog gets displayed.

![image](/posts/images/AddPaymentDetails.PNG)

This modal is actually really simple because all we need is a `form` with empty place holders that will be filled by `Stripe.js`.
...
In the script section of this page is where most of the magic will take place. First we need to include the JavaScript library from stripe. Then we create a new Stripe object with the `PublicKey` that we can get from the configuration, then we added some custom styling which of course can be adjust to the sites look and feel and is described in detail in the docs page of Stripe.
...
As explained above we need to create a token through the `Stripe.js` library and then submit that to our backend. So how do we do this? Just a few lines of JavaScript really. 

We add an event listener to our form on submit and prevent the default action so that is not posted to the backend. The stripe object that we created above offers a `createToken` function that returns a promise. We pass in the `card` element that we created while bootstrapping the form and as a result of calling the function we get the token. I said it was simple.
...
What really happened behind the scenes was that `Stripe.js` called the Stripe API tokens endpoint with the credit card information. So nothing magical at all, just a simple XHR call to the API that returned the token to us.

![image](/posts/images/StripeCreateToken.PNG)

We can then work with this token, send it to our backend...
...
...and create a card for our customer.
